...
function change(v){
	// alert("change");
	expire_hour = 1; // 過期日期(天)
 	var d = new Date();
    d.setTime(d.getTime() + (expire_hour *  60 * 60 * 1000));
    var expires = "expires=" + d.toGMTString();
    if(v>0){
    	var cookievalue=document.getElementById(s[v-1]).value
    	document.cookie = s[v-1]+"="+cookievalue + "; " + expires + ";";
    }

    //document.getElementById(s[v-1]).value
	var str="0";
	for(i=0;i<v;i++){
	...
	//自動填入地址
	if(!document.getElementById('s_address').readOnly){
		if(document.getElementById(s[0]).value!="省份"){
			document.getElementById('s_address').value=document.getElementById(s[0]).value;
			if(document.getElementById(s[1]).value!="地级市"){
				document.getElementById('s_address').value+=document.getElementById(s[1]).value;
			}
		}
	}
}
