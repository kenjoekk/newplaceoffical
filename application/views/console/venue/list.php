...
<div class="row clearfix">
  <?php foreach ($list as $key => $value) { ?>
    <div class="col-lg-4 col-md-4 col-sm-6 col-xs-12">
        <div class="card venue-card" style="<?php echo "background-image:url('".$value['img_url'][0]."')"?>">
            <div class="header">
                
            </div>
            <div class="body">
                <h3><?=$value['english_name']?></h3>
...
<script>
  $(function(){
    $('.btn-remove').on('click',function(){
      ...
      swal({
        ...
        var id = $(that).closest('.footer').data('id');
          $.post("<?=base_url('consoleApi/removeVenue')?>",{
            id:id
          },function(data){
            location.reload();
          },'json');
    ...
    $('.card-move').on('click',function(){
      ...
      $.post("<?=base_url('consoleApi/moveVenue')?>",{
        id:id,
        type:type
      },function(data){
        location.reload();
